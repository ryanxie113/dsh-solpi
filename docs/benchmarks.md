...
## T2 vanilla 对照：插件税的任务形状敏感性（2026-09-15）

vanilla 臂补跑 N=3（84.7/86.0/64.5s，12/12 全绿）与 solpi 同任务对照，得到本基准
最重要的边界事实之一：

| 组 | wall | requests | 质量 |
|---|---|---|---|
| vanilla ×3 | **78.4s [64.5–86.0]** | 15.0 | 12/12 ✓ |
| solpi coach v1 ×3 | 288.9s | 12.7 | 12/12 ✓ |
| solpi coach v2 ×3 | 240.8s [235–248] | 12.3 | 12/12 ✓ |

**T2 型任务（从零搭建小项目+自验循环）solpi 反而有 +208% 税**，即使 coach v2 已让
融合高采纳。机理解读：小项目一把梭搭建无历史上下文可压（OCC 无收益）、任务短到
gate 全程 deferred_economic；而逐 mutation 验证语义在"写了就测"的高频失败-重试环中
放大交互轮次成本（v2 轮 then_run failed 占比 ~45%）。
...
|---|---|---|
| 红→绿诊断修复（中等规模） | **−58%** | T1 |
| 38KB 记忆问答 | **−33%** 保真无损 | T3 |
| 12 模块长程大任务 | **−80%** | T4 |
| 链式编辑（中规模） | ≈−48%（行为方差内） | T2 早轮 |
| **从零搭小项目自验** | **+208% 税** | 本对照 |

诚实的附带声明：vanilla 与 solpi 轮次非同时段交错采样（GLM Flash 存在时段速度波动），
方向性结论可信（差距远超方差带），精确倍数需后续交错复测。
README「何时启用」已同步标注此最差场景。
